...
def randomForest(t_student,PCA_funtion,nameMatrix, graf, meth):
    ...
    pd.DataFrame(accuracy_t_c).to_csv("./data/"+ nameMatrix + "/" + nameMatrix + 'randomForest_accuracy_t_c.csv')
    pd.DataFrame(precision_t_c).to_csv("./data/"+ nameMatrix + "/" + nameMatrix + 'randomForest_precision_t_c.csv')
    pd.DataFrame(accuracy_t_b).to_csv("./data/"+ nameMatrix + "/" + nameMatrix + 'randomForest_accuracy_t_b.csv')
    pd.DataFrame(precision_t_b).to_csv("./data/"+ nameMatrix + "/" + nameMatrix + 'randomForest_precision_t_b.csv')
    ...
    for n in range(100,300,25):
        radomForest = RandomForestClassifier(n_estimators=n, max_features=0.1, random_state=42)
        accuracy_PCA_c.append(cross_val_score(radomForest, x_PCA, y_PCA, cv = 5, scoring='accuracy').mean())
    ...
    pd.DataFrame(accuracy_PCA_b).to_csv("./data/"+ nameMatrix + "/" + nameMatrix + 'randomForest_accuracy_PCA_b.csv')
    pd.DataFrame(precision_PCA_b).to_csv("./data/"+ nameMatrix + "/" + nameMatrix + 'randomForest_precision_PCA_b.csv')
